...
impl ChessEngine {
    pub fn new() -> Self {
        let tt_size = 1 << 21; // 2M entries for stronger play
        Self {
    ...
    fn negamax(&mut self, board: &Board, depth: u8, mut alpha: i16, beta: i16, ply: usize, do_null: bool) -> i16 {
        ...
        for chess_move in moves {
            ...
            // Late move reductions - slightly more aggressive for stronger play
            if move_count > 4 && depth >= 3 && 
               ...
               new_board.checkers().popcnt() == 0 { // Doesn't give check
                ...
                let reduction = if move_count > 10 { 2 } else { 1 };
    ...
    loop {
        ...
        match parse(input) {
            ...
            Ok((
                ...
                let search_depth = depth.unwrap_or(12) as u8;
                ...
                if engine.transposition_table.iter().filter(|e| e.is_some()).count() > 1_700_000 {
